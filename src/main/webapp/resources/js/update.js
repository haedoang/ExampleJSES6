...
//staff 졸업일
const sGraduateDay = document.querySelector("input[name='graduateDay']");
...
for(var y = new Date().getFullYear(); y >= 1980 ; y-- ){
	...
	opEle.value=y;
	opEle.innerText = y;
	if(opEle.value==sGraduateDay.value.substring(0,4)){
...
for(var m=1; m <= 12; m++){
	...
	if(opEle.value==sGraduateDay.value.substring(5,7)){
		opEle.setAttribute("selected","selected");
	}
...
for(var d=1; d<=31; d++){
	...
	if(opEle.value==sGraduateDay.value.substring(8,10)){
		opEle.setAttribute("selected","selected");
	}
...
function formCheck(){
	...
	var obj ={};
	obj.staffNo = document.querySelector("input[name='staffNo']").value;
	//1) name check;
	const stNameEle = document.querySelector("input[name='name']");
	//입력안할 시 
	if(stNameEle.value===''){
		alert("이름을 입력하세요");
		return false;
	...
	//2) 요청페이지 주소
	xhr.open("POST","/mvc-test/board/update/doUpdate.do",true);
...
const delBtn = document.querySelector("input[name='delete']");
delBtn.onclick = ()=>{
	const staffNo = document.querySelector("input[name='staffNo']").value;
	
	var xhr = new XMLHttpRequest();
	xhr.onreadystatechange = function(){
		if(xhr.readyState==4){
			if(xhr.status==200){
				alert("삭제완료! 검색으로 돌아갑니다.");
	...
	//2) 요청페이지 주소
	xhr.open("GET","/mvc-test/board/update/doDelete.do?staffNo="+staffNo,true);
